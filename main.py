...
def download_icon_data(run_date, run_hour):
    ...
    if len(existing_files) >= len(VARIABLES):
        print(f"Tutti i GRIB già presenti ({len(existing_files)}/{len(VARIABLES)})")
        return True
    ...
    if requests.head(base_url, timeout=10).status_code != 200:
        print(f"Run {run_date}{run_hour} non disponibile")
        return False
...
def process_data():
    ...
    for var in VARIABLES:
        ...
        if os.path.exists(grib_path):
            ...
            print(f"Manca {var}.grib → ritenta download")
            if not download_icon_data(run_date, run_hour):
                raise RuntimeError(f"{var}.grib mancante dopo ritentativo")
    
    if 'HSURF' not in data: raise RuntimeError("HSURF.grib mancante")
    
    hsurf_da = list(data['HSURF'].data_vars.values())[0]
    lat_grid, lon_grid = build_latlon_from_hsurf(hsurf_da)
    
    hsurf_grid = hsurf_da.isel(time=0).values if hsurf_da.ndim==3 else hsurf_da.values
    ...
    for city, info in venues.items():
        try:
            ...
            # (estrazione variabili e calcoli già come nel tuo script)
            # ...→ codice come sopra per t2m_final, rh2m, tp_h, clct, clcl, ecc
            # Salvataggio JSON triorario/giornaliero → come nel tuo script
            processed += 1
        except Exception as e:
            print(f"Errore {city}: {e}")
            continue
    
    # CHECK finale JSON: tutti i file generati?
    missing_files = [c for c in venues if not os.path.exists(f"{output_dir}/{c}.json")]
    if missing_files:
        print(f"Attenzione, mancano JSON per {len(missing_files)} città: {missing_files[:10]} ...")
    else:
        print(f"Tutti i JSON generati correttamente ({processed}/{len(venues)})")
