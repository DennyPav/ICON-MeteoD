...
def calculate_daily_summaries(hourly_data, clct_daily, clcl_daily, clcm_daily,
                              clch_daily, tp_h_daily, lpi_daily, cape_daily,
                              uh_daily, season, season_thresh):
    daily_summaries = []
    days_data = {}
    ...
    # Ordina i giorni e scarta SEMPRE l’ultimo
    ordered_days = sorted(days_data.keys())
    complete_days = ordered_days[:-1]  # esclude l’ultimo giorno

    for day in complete_days:
        day_records = days_data[day]
        ...
        tp_total_day = sum([record["p"] for _, record in day_records])
        ...
        weather_day = classify_weather(
            t_mean, r_mean,
            clct_daily[first_idx], clcl_daily[first_idx],
            clcm_daily[first_idx], clch_daily[first_idx],
            tp_total_day, v_mean,
            lpi_daily[first_idx], cape_daily[first_idx],
            uh_daily[first_idx], season, season_thresh,
            timestep_hours=24
        )
